...
## Unreleased
- Editor v2 ligero: separa el editor del Home, mantiene notas largas en un textarea uncontrolled para evitar copiar/re-renderizar el contenido completo por tecla, toma snapshots solo al guardar y recupera la hoja rayada históricamente alineada (32px/17px/24px) como capa visual reemplazable. Se elimina la implementación legacy restante del editor del árbol activo.
