...
import { REPLICA_RETIRED_ATTACHMENT_KIND } from './replicaRetiredAttachmentCodec.ts'
...
function isReplicaMetadataBlock(block: EditorSurfaceBlock): boolean {
  return block.kind === REPLICA_ATTACHMENT_PRESENTATION_KIND
    || block.kind === REPLICA_RETIRED_ATTACHMENT_KIND
}

/**
 * Durable replica metadata is not part of the visual/editor order.
