...
test('Android back runtime closes the active rebuild layer through its explicit safe back contract', () => {
  ...
  assert.match(editorSurface, /<ActiveSurface[\s\S]*\{\.\.\.surfaceProps\}[\s\S]*\/>/)
  assert.match(editorSurface, /OanixTextLineRuntimeProvider/)
  assert.match(editorSurface, /flushOanixTextLineEditors/)
  assert.doesNotMatch(editorSurface, /installOanixTextBehaviorBridge|behaviorRevision/)
