...
import { REPLICA_RETIRED_ATTACHMENT_KIND } from '../src/features/editor/replicaRetiredAttachmentCodec.ts'
import {
...
test('replica flow split keeps durable metadata out of visual order', () => {
  const blocks = [
    block('a'),
    block('meta-a', REPLICA_ATTACHMENT_PRESENTATION_KIND),
    block('retired-a', REPLICA_RETIRED_ATTACHMENT_KIND),
    block('b'),
...
test('replica flow insertion at end stays before trailing hidden records', () => {
  const blocks = [
    block('a'),
    block('b'),
    block('meta-a', REPLICA_ATTACHMENT_PRESENTATION_KIND),
