...
test('plain batch stores in controlled parallelism and commits document blocks once', async () => {
  ...
  const result = await insertOanixImageBatch({
    ...
    savePlainSnapshot: async (snapshot) => snapshot.title === 'Nota' && snapshot.text === '', removeAttachment: async () => true,
    onProgress: (value) => progress.push(`${value.stage}:${value.completed}/${value.total}`), createId: deterministicId,
  })
...
test('plain batch keeps long suffix after the whole image group and leaves editable separators', async () => {
  ...
  assert.deepEqual(imageIndexes, [1, 3, 5])
