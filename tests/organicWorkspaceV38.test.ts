...
test('folders, tags and notes finish reordering automatically after release', () => {
  ...
  assert.match(noteGesture, /type GesturePhase = 'pressing' \| 'scrolling' \| 'dragging'/)
  assert.match(noteGesture, /document\.addEventListener\('pointerup', finishDrag, true\)/)
  assert.match(noteGesture, /setPointerCapture\(event\.pointerId\)/)
  ...
  assert.doesNotMatch(noteGesture, /touchstart|touchmove|touchend|touchcancel|PRESS_ARM_GRACE_MS|finishAutomaticMode|dispatchDragStart|new PointerEvent/)
