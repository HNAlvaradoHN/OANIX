...
test('folders, tags and notes finish reordering automatically after release', () => {
  ...
  assert.match(noteGesture, /const LONG_PRESS_MS = 280/)
  assert.match(noteGesture, /document\.addEventListener\('pointerup', finishGesture/)
  assert.match(noteGesture, /document\.addEventListener\('pointermove', onPointerMove, \{ capture: true, passive: false \}\)/)
  assert.match(noteGesture, /persistNoteOrder\(nextOrder\)/)
  assert.match(noteGesture, /createPlaceholder/)
  assert.match(noteGesture, /createClone/)
  assert.match(noteGesture, /setPointerCapture/)
