...
test('legacy workspace css is not eagerly loaded when v2 is authoritative', () => {
  ...
  assert.doesNotMatch(legacyGate, /v383WorkspaceVisual\.css|workspaceStateContract\.css/)
  assert.match(v383WorkspaceVisualRuntime, /import '\.\/v383WorkspaceVisual\.css'[\s\S]*import '\.\/workspaceStateContract\.css'/)
  assert.match(legacyGate, /editorTrailingWorkspace\.css'[\s\S]*V383WorkspaceVisualRuntime[\s\S]*workspaceRefinements\.css'/)
