...
test('Home exposes confirmed deletion for notes, folders and tags without nested buttons', () => {
  ...
  assert.match(app, /<div key=\{note\.id\} className="rebuild-note-row">/)
  assert.match(app, /className="rebuild-note-row__open"/)
  assert.match(app, /className="rebuild-note-row__delete"/)
