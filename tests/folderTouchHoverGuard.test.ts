...
test('touch folder interactions neutralize latched hover transforms', () => {
  ...
  assert.match(block, /\.oanix-folder-rail__item:hover:not\(\.is-selected\)[\s\S]*transform: none !important/)
  assert.match(block, /\.oanix-folder-rail__item\.is-selected:hover[\s\S]*translateY\(-2px\) translateZ\(18px\)/)
