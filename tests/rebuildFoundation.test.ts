...
test('typing stays local and persistence happens only at idle or close save boundaries', () => {
  assert.match(rebuild, /<EditorSurface/)
  assert.match(rebuild, /async function saveEditorSnapshot\(snapshot: EditorSurfaceSnapshot\)/)
  ...
  assert.match(editorSurface, /selectedSurface\.capabilities\.richBlocks/)
