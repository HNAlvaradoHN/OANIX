...
test('ghost conserva fallback autónomo y overlay mantiene la apariencia real de la nota', () => {
  ...
  assert.doesNotMatch(css, /body > \.note-row\.oanix-mobile-note-drag-ghost,[\s\S]*.notes-shell > \.note-row\.oanix-mobile-note-drag-overlay\s*\{[\s\S]*background:\s*rgba\(18,18,35,\.96\) !important/)
})
