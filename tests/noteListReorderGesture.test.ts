...
test('overlay visible usa el mismo patrón fixed left top de carpetas sin sustituir Sortable', () => {
  ...
  assert.doesNotMatch(runtime, /overlay\.removeAttribute\('data-reorder-note-id'\)/)
  assert.doesNotMatch(runtime, /overlay\.style\.setProperty\('background', style\.background/)
...
test('identidad visual se congela por note id durante el drag y se restaura antes de persistir', () => {
  ...
  assert.match(runtime, /--oanix-note-stable-color/)
