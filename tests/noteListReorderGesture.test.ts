...
test('la tarjeta visible usa un overlay independiente y sigue directamente el dedo', () => {
  ...
  assert.doesNotMatch(runtime, /createDragOverlay\(event\.item/)
  assert.match(runtime, /createDragOverlay\(touchGesture\.item/)
...
test('orden persiste y sincroniza React sin remonte completo en el camino exitoso', () => {
  ...
  assert.match(runtime, /pendingPersistOrder = \[\.\.\.nextOrder\]/)
  assert.match(runtime, /const updatedNotes = await persistNoteOrder\(orderToPersist, \(\) => !disposed && pendingPersistOrder === null\)/)
  assert.match(runtime, /oanix:note-order-preview/)
  assert.match(workspace, /orderedSet\.has\(note\.id\) \? orderedNotes\[orderedIndex\+\+\] : note/)
  ...
  assert.match(runtime, /try \{[\s\S]*persistNoteOrder\(orderToPersist,[\s\S]*catch \{[\s\S]*oanix:workspace-refresh/)
  assert.match(runtime, /if \(persistLoop\) return/)
  assert.match(runtime, /while \(!disposed && pendingPersistOrder\)/)
  assert.match(runtime, /stopScrollMomentum\(\)[\s\S]*suppressClickUntil = 0[\s\S]*beginGesture/)
